    ...
    <form method="POST" action="{{ route('management.menu-items.update', $menuItem->id) }}" id="mainForm" novalidate>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm mb-6 overflow-hidden">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                {{-- ✅ FIXED: uses current_unit_cost and unit_of_measurement --}}
                    ...
                    <select name="inventory_item_id" id="inventory_item_id"
                        ...
                        @foreach($inventoryItems as $item)
                            <option value="{{ $item->id }}"
                                    data-unit-cost="{{ $item->current_unit_cost ?? 0 }}"
                                    data-base-unit="{{ $item->unit_of_measurement }}"
                                    data-selling-price="{{ $item->selling_price ?? 0 }}"
                                    {{ old('inventory_item_id', $menuItem->inventory_item_id) == $item->id ? 'selected' : '' }}>
                                {{ $item->name }} ({{ $item->unit_of_measurement }} — {{ number_format($item->current_unit_cost ?? 0, 2) }} UGX)
                            </option>
                        @endforeach
                    </select>
        ...
        <div id="recipeSection" class="bg-white rounded-2xl border border-gray-200 shadow-sm mb-6 overflow-hidden {{ $isBeverage ? 'hidden' : '' }}">
            ...
            <div class="p-6">
                <div id="ingredientsEmptyState" class="flex flex-col items-center justify-center py-8 text-center {{ count($recipeIngredients) > 0 ? 'hidden' : '' }}">
                ...
                <div id="ingredientsContainer" class="space-y-3">
                    {{-- ✅ FIXED: loop over $recipeIngredients (pre-resolved FIFO costs) --}}
                    @foreach($recipeIngredients as $index => $ri)
                        @php
                            $totalCost = $ri['quantity'] * $ri['unit_cost'] * (1 + $ri['wastage_percentage'] / 100);
                        @endphp
                        <div class="ingredient-row flex items-center gap-3 px-3 py-2 bg-gray-50 rounded-lg border border-gray-200"
                             data-ingredient-idx="{{ $index }}">
                            <input type="hidden" class="ingredient-recipe-id"    name="ingredients[{{ $index }}][recipe_id]"          value="{{ $ri['recipe_id'] }}">
                            <input type="hidden" class="ingredient-inventory-id" name="ingredients[{{ $index }}][inventory_item_id]"  value="{{ $ri['inventory_item_id'] }}">
                            <input type="hidden" class="ingredient-base-unit"    name="ingredients[{{ $index }}][base_unit]"           value="{{ $ri['base_unit'] }}">
                            <input type="hidden" class="ingredient-unit-cost"    name="ingredients[{{ $index }}][unit_cost]"           value="{{ $ri['unit_cost'] }}">
                            <input type="hidden" class="ingredient-unit-selected" name="ingredients[{{ $index }}][unit]"              value="{{ $ri['unit'] }}">
                            <input type="hidden" class="ingredient-wastage-val"  name="ingredients[{{ $index }}][wastage_percentage]"  value="{{ $ri['wastage_percentage'] }}">
                            <input type="hidden" class="ingredient-quantity"     name="ingredients[{{ $index }}][quantity]"            value="{{ $ri['quantity'] }}">

                            <div class="flex-1">
                                <p class="text-sm font-medium text-gray-800">{{ $ri['name'] }}</p>
                                <p class="text-xs text-gray-400">{{ number_format($ri['quantity'], 3) }} {{ $ri['unit'] }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-xs text-gray-400">Unit cost
                                    @if($ri['cost_source'] === 'last_known')
                                        <span class="text-amber-500">(last known)</span>
                                    @elseif($ri['cost_source'] === 'department_stock')
                                        <span class="text-blue-500">(dept stock)</span>
                                    @endif
                                </p>
                                <p class="text-sm font-medium text-gray-700">{{ number_format($ri['unit_cost'], 0) }} UGX / {{ $ri['base_unit'] }}</p>
                            </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm mb-6 overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50">
                ...
                <div class="flex items-center gap-1 bg-white rounded-lg p-0.5 border border-gray-200">
                    <button type="button"
                            class="pricing-mode-toggle text-xs px-3 py-1 rounded-md transition-all {{ old('pricing_mode', $menuItem->age_margins > 0 ? '' : 'active') }}"
                            data-mode="price">
                        Set Price
                    </button>
                    <button type="button"
            ...
            <div class="p-6 space-y-4">
                <div class="price-mode-field {{ old('pricing_mode', $menuItem->age_margins > 0 ? 'hidden' : '') }}">
                    ...
                    <input type="number" name="selling_price" id="selling_price"
                           ...
                           value="{{ old('selling_price', $menuItem->selling_price) }}">
                ...
                <div class="margin-mode-field {{ old('pricing_mode', $menuItem->age_margins > 0 ? '' : 'hidden') }}">
                    ...
                    <input type="number" name="desired_margin" id="desired_margin"
                           ...
                           value="{{ old('desired_margin', $menuItem->age_margins ?? '') }}">
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-3 gap-3 bg-orange-50 rounded-lg px-3 py-2">
                    <div>
                        ...
                        <p class="text-sm font-semibold text-orange-700" id="glovoPriceDisplay">
                            {{ number_format($menuItem->glovo_selling_price ?? ($menuItem->selling_price * 1.2), 0) }} UGX
                        </p>
                    </div>
                    <div>
                        <span class="text-xs text-gray-500">Glovo Commission</span>
                        <p class="text-sm font-semibold text-red-600" id="glovoCommissionDisplay">
                            {{ number_format($menuItem->glovo_commission ?? 0, 0) }} UGX
                        </p>
                    </div>
                    <div>
                        <span class="text-xs text-gray-500">Final Margin (Glovo)</span>
                        <p class="text-sm font-semibold text-emerald-600" id="finalMarginDisplay">
                            {{ number_format($menuItem->final_margin ?? 0, 0) }} UGX
                        </p>
                    </div>
                </div>
    ...
    <div class="bg-white rounded-2xl max-w-2xl w-full overflow-hidden shadow-xl">
        ...
        <div class="p-6">
            ...
            <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
                ...
                <button type="button" id="confirmAddIngredient"
    ...
    <div class="bg-white rounded-2xl max-w-md w-full overflow-hidden shadow-xl">
        ...
        <div class="p-6">
            ...
            <p class="text-xs text-red-500 mb-4">
                ...
                This will also delete all recipe ingredients. This action cannot be undone.
            </p>
            <form method="POST" action="{{ route('management.menu-items.destroy', $menuItem->id) }}">
                @csrf
                @method('DELETE')
...
$(document).ready(function () {
    ...
    let ingredientCounter = {{ count($recipeIngredients) }};
    let currentMaterialCost = parseFloat('{{ $menuItem->m_cost ?? 0 }}') || 0;
    let isBeverage = {{ $isBeverage ? 'true' : 'false' }};

    // ✅ FIXED: read current_unit_cost from selected option
    let selectedInventoryCost = parseFloat(
        $('#inventory_item_id option:selected').data('unit-cost')
    ) || 0;
    ...
    function updateBeveragePricing() {
        ...
        let ageCost       = netPrice > 0 ? (unitCost / netPrice) * 100 : 0;
        ...
        $('#age_cost_hidden').val(ageCost.toFixed(2));
    ...
    function updateRecipePricing() {
        ...
        if (currentMaterialCost > 0) {
            if (pricingMode === 'margin' && desiredMargin > 0 && desiredMargin < 100) {
                ...
                desiredMargin     = netPrice > 0 ? (actualMarkup / netPrice) * 100 : 0;
        ...
        let ageCost       = netPrice > 0 ? (currentMaterialCost / netPrice) * 100 : 0;
    ...
    function calculateTotalMaterialCost() {
        ...
        $('#materialCostDisplay, #summaryMaterialCost').text(formatMoney(total));
    ...
    // =====================================================
    // BEVERAGE INVENTORY CHANGE
    // =====================================================
    $('#inventory_item_id').on('change', function() {
        // ✅ FIXED: Select2-safe option reading
        let selectedId = $(this).val();
        let selOpt     = $('#inventory_item_id option[value="' + selectedId + '"]');
        ...
        if (unitCost > 0) {
            $('#inventoryUnitCostDisplay').text('Unit cost: ' + formatMoney(unitCost) + ' per ' + baseUnit);
        } else {
            $('#inventoryUnitCostDisplay').text('');
        }
    ...
    // ✅ FIXED: Select2-safe cost preview
    ...
    // ✅ FIXED: Select2-safe confirm, plus baseUnit guard
    $('#confirmAddIngredient').on('click', function() {
        ...
        if (!baseUnit) {
            alert('This ingredient is missing a unit of measurement. Please check the inventory item.');
            return;
        }
        ...
        let rowHtml = `
            <div class="ingredient-row flex items-center gap-3 px-3 py-2 bg-gray-50 rounded-lg border border-gray-200" data-ingredient-idx="${idx}">
                ...
                <div class="text-right min-w-[100px]">
                    <p class="text-xs text-gray-400">Total</p>
                    <p class="text-sm font-semibold text-gray-900 ingredient-total-display">${formatMoney(totalCost)}</p>
                </div>
                <button type="button" class="remove-ingredient w-8 h-8 rounded-lg bg-red-50 hover:bg-red-100 text-red-500 transition-colors flex items-center justify-center">
                    <i class="fas fa-trash text-xs"></i>
                </button>
    ...
    $(document).on('click', '.remove-ingredient', function() {
        if (confirm('Remove this ingredient from the recipe?')) {
            ...
            // Track deleted recipe IDs so controller can remove them
            if (recipeId) {
                if ($('#deleteRecipeItems').length === 0) {
                    $('<input>').attr({
                        type: 'hidden',
                        id: 'deleteRecipeItems',
                        name: 'delete_recipe_items'
                    }).val('').appendTo('#mainForm');
                }
                let current = $('#deleteRecipeItems').val();
                $('#deleteRecipeItems').val(current + (current ? ',' : '') + recipeId);
            }
    ...
    $('#mainForm').on('submit', function(e) {
        ...
        if (!isBeverage && $('.ingredient-row').length === 0) {
