...
@section('title', 'Kitchen Order Tickets')

@section('content')
<div class="max-w-7xl mx-auto px-4 pb-8">

    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Kitchen Order Tickets</h1>
        <p class="text-sm text-gray-500 mt-0.5">Manage food orders from waiters</p>
    </div>
...
<script>
    ...
    function printTicket(ticketId) {
        window.open('/kitchen/tickets/' + ticketId + '/print', '_blank', 'width=400,height=500');
    }

    function completeTicket(ticketId) {
        if (!confirm('Mark this order as completed?')) return;

        fetch('/kitchen/tickets/' + ticketId + '/complete', {
            method: 'POST',
            headers: {
