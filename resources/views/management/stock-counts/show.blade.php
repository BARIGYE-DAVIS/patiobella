...
<div class="space-y-4" id="printArea">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex items-center justify-between flex-wrap gap-4">
            ...
            <div class="flex gap-2 no-print">
                ...
                <a href="{{ route('management.stock-counts.review', $stockCount->id) }}"
                   class="px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg text-sm transition flex items-center gap-2">
    ...
    {{-- Action Buttons for In Progress (Cancel only) --}}
    @if($stockCount->status === 'in_progress')
    <div class="flex justify-end gap-3 no-print">
        <form action="{{ route('management.stock-counts.cancel', $stockCount->id) }}" method="POST" onsubmit="return confirm('Cancel this stock count? This action cannot be undone.')">
            @csrf
            @method('DELETE')
