        ...
        <tbody>
            ...
            @foreach($orders as $order)
                ...
                @foreach($order->items as $item)
                    ...
                    <tr>
                        {{-- Invoice # - Only show on first item row --}}
                        @if($firstItem)
                            <td class="invoice-number" style="vertical-align: top;" rowspan="{{ $itemCount }}">
                                {{ $order->order_number }}
                            </td>
                            <td style="vertical-align: top;" rowspan="{{ $itemCount }}">
                        ...
                        {{-- Payment Details - Only show on first item row --}}
                        @if($firstItem)
                            <td style="vertical-align: top;" rowspan="{{ $itemCount }}" class="text-right">
                                UGX {{ number_format($order->amount_paid ?? $order->total_amount, 0) }}
                            </td>
                            <td style="vertical-align: top;" rowspan="{{ $itemCount }}" class="text-right">
                                UGX {{ number_format($order->change_amount ?? 0, 0) }}
                            </td>
                        @endif
                    </tr>
