    ...
    <div class="p-6">
        <form method="POST" action="{{ route('restaurant.requisitions.store') }}" id="requisitionForm">
            ...
            <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
                <a href="{{ route('restaurant.requisitions.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-5 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
...
function generatePreviewHTML() {
    ...
    const requesterName = currentUser ? (escapeHtml(currentUser.first_name || '') + ' ' + escapeHtml(currentUser.last_name || '')) : 'N/A';
    const signatureUrl = (currentUser && currentUser.signature_path) ? storageBaseUrl + '/' + currentUser.signature_path : null;

    // Requested By block - Name ABOVE signature
    let requestedByBlock = `
        <div style="text-align: center;">
            <div style="font-weight: bold; font-size: 14px; color: #1f2937; margin-bottom: 8px;">${requesterName}</div>
            <div style="font-size: 11px; color: #6b7280; margin-bottom: 12px;">Requested By</div>
    `;

    if (signatureUrl) {
        requestedByBlock += `
            <img src="${signatureUrl}" style="max-height: 60px; max-width: 200px; object-fit: contain; display: block; margin: 0 auto;">
        `;
    } else {
        requestedByBlock += `
            <div style="border-top: 1px solid #cbd5e1; width: 180px; margin: 0 auto;"></div>
            <div style="font-size: 10px; color: #9ca3af; font-style: italic; margin-top: 8px;">No signature uploaded</div>
        `;
    }

    requestedByBlock += `
            <div style="font-size: 10px; color: #9ca3af; margin-top: 12px;">${formattedDate}</div>
        </div>
    `;

    // Approved By block - Name ABOVE signature (placeholder for now)
    let approvedByBlock = `
        <div style="text-align: center;">
            <div style="font-weight: bold; font-size: 14px; color: #1f2937; margin-bottom: 8px;">_________________________</div>
            <div style="font-size: 11px; color: #6b7280; margin-bottom: 12px;">Approved By</div>
            <div style="border-top: 1px dashed #cbd5e1; width: 180px; margin: 0 auto;"></div>
            <div style="font-size: 10px; color: #9ca3af; font-style: italic; margin-top: 8px;">(To be signed upon approval)</div>
            <div style="font-size: 10px; color: #9ca3af; margin-top: 12px;">&nbsp;</div>
        </div>
    `;

    return `
        <div style="font-family: Arial, Helvetica, sans-serif; max-width: 100%; margin: 0 auto; padding: 20px;">
            <!-- Header -->
            <div style="text-align: center; margin-bottom: 24px; border-bottom: 3px solid #f97316; padding-bottom: 16px;">
                <h2 style="margin: 0; color: #111827; font-size: 22px;">RESTAURANT REQUISITION FORM</h2>
                <p style="margin: 6px 0 0 0; color: #6b7280; font-size: 12px;">Department Requisition Slip</p>
            </div>
            ...
            <div style="margin-bottom: 24px;">
                <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                    <tr>
                        <td style="padding: 6px 12px 6px 0; width: 33%;"><strong>Date Created:</strong> ${formattedDate}</td>
                        <td style="padding: 6px 12px 6px 0; width: 33%;"><strong>Department:</strong> Restaurant</td>
                        <td style="padding: 6px 12px 6px 0; width: 33%;"><strong>Requisition Type:</strong> ${escapeHtml(requisitionTypeLabel)}</td>
                    </tr>
                    <tr>
                        <td style="padding: 6px 12px 6px 0;"><strong>Date Needed:</strong> ${dateNeeded || 'Not specified'}</td>
                        <td style="padding: 6px 12px 6px 0;"><strong>Status:</strong> Pending</td>
                        <td style="padding: 6px 12px 6px 0;"><strong>Requested By:</strong> ${requesterName}</td>
                    </tr>
                </table>
            </div>
            ...
            <!-- Notes Section -->
            ${departmentNotes ? `
            <div style="margin-bottom: 20px; padding: 12px; background: #fefce8; border-left: 4px solid #f97316; border-radius: 4px;">
                <strong style="font-size: 12px;">📝 Notes / Instructions:</strong>
                <p style="margin: 8px 0 0 0; color: #4b5563; font-size: 12px;">${escapeHtml(departmentNotes)}</p>
            </div>
            ` : ''}
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 28px; font-size: 12px;">
                <thead>
                    <tr>
                        <th style="border: 1px solid #d1d5db; padding: 10px 8px; background: #f97316; color: #ffffff; text-align: center; font-weight: 600;">#</th>
                        <th style="border: 1px solid #d1d5db; padding: 10px 8px; background: #f97316; color: #ffffff; text-align: left; font-weight: 600;">Item Name</th>
                        <th style="border: 1px solid #d1d5db; padding: 10px 8px; background: #f97316; color: #ffffff; text-align: center; font-weight: 600;">Quantity</th>
                        <th style="border: 1px solid #d1d5db; padding: 10px 8px; background: #f97316; color: #ffffff; text-align: center; font-weight: 600;">Unit</th>
                    </tr>
                </thead>
                <tbody>
                    ${items.map((item, idx) => `
                        <tr>
                            ...
                            <td style="border: 1px solid #d1d5db; padding: 8px; text-align: center;">${parseFloat(item.quantity).toFixed(2)}</td>
                            <td style="border: 1px solid #d1d5db; padding: 8px; text-align: center;">${escapeHtml(item.metrics) || 'pcs'}</td>
                        </tr>
                    `).join('')}
                    ${items.length === 0 ? `
                        <tr>
                            <td colspan="4" style="border: 1px solid #d1d5db; padding: 30px; text-align: center; color: #9ca3af;">No items added</td>
                        </tr>
                    ` : ''}
                </tbody>
            </table>

            <!-- Signatures Section - Horizontal Row, Name above Signature -->
            <div style="margin-top: 32px;">
                <table style="width: 100%; border-collapse: collapse;">
                    <tr>
                        <td style="width: 50%; padding: 0 24px 0 0; vertical-align: top; border-right: 1px solid #e5e7eb;">
                            ${requestedByBlock}
                        </td>
                        <td style="width: 50%; padding: 0 0 0 24px; vertical-align: top;">
                            ${approvedByBlock}
                        </td>
                    </tr>
                </table>
            ...
            <!-- Footer -->
            <div style="margin-top: 32px; text-align: center; font-size: 9px; color: #9ca3af; border-top: 1px solid #e5e7eb; padding-top: 16px;">
                <p style="margin: 0;">This is a system-generated requisition. Please verify all items before approval.</p>
                <p style="margin: 4px 0 0 0;">Generated on: ${formattedDate}</p>
            </div>
        </div>
    `;
}
    ...
    function printPreview() {
        ...
        printWindow.document.write(`
            <html>
                <head>
                    <title>Restaurant Requisition Preview</title>
                    <style>
    ...
    function downloadPreviewPDF() {
        ...
        const opt = {
            ...
            filename: 'Restaurant_Requisition_Preview_' + new Date().toISOString().slice(0,19) + '.pdf',
