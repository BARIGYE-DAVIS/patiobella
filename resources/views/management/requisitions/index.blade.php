...
<div class="space-y-6">
        ...
        <div class="border-b border-gray-200">
            <nav class="flex -mb-px">
                ...
                <a href="{{ route('management.requisitions.index') }}?tab=all" 
                   class="px-6 py-3 text-sm font-medium {{ request('tab') == 'all' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-700' }}">
                    All Requisitions
                </a>
            </nav>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="p-4 border-b">
            <form method="GET" action="{{ route('management.requisitions.index') }}" class="flex flex-wrap gap-4 items-end">
                ...
                <div class="flex space-x-2">
                    ...
                    <a href="{{ route('management.requisitions.index') }}?tab={{ request('tab', 'pending') }}" 
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="bg-gradient-to-r from-gray-800 to-gray-700 px-6 py-4">
            <h2 class="text-xl font-bold text-white">
                @if(request('tab', 'pending') == 'pending') Requisitions Pending Approval
                @elseif(request('tab') == 'approved') Approved Requisitions
                @elseif(request('tab') == 'rejected') Rejected Requisitions
                @else All Requisitions
                @endif
            </h2>
        </div>
            ...
            <table class="min-w-full border border-gray-200 rounded-lg">
                ...
                <tbody>
                    ...
                    <tr class="border-b hover:bg-gray-50">
                        <td class="p-3 border font-semibold">{{ $req->requisition_number }}</td>
                        <td class="p-3 border">{{ $req->store->name ?? 'N/A' }}</td>
                        <td class="p-3 border">
                            {{ $req->requestedBy ? $req->requestedBy->first_name . ' ' . $req->requestedBy->last_name : 'N/A' }}
                        </td>
                        <td class="p-3 border text-center">{{ $req->created_at->format('Y-m-d') }}</td>
                        <td class="p-3 border text-center">{{ $req->date_needed ? date('Y-m-d', strtotime($req->date_needed)) : 'N/A' }}</td>
                        <td class="p-3 border text-center">{{ $req->items->count() }}</td>
                        <td class="p-3 border text-right">{{ number_format($req->items->sum('quantity_requested'), 2) }}</td>
