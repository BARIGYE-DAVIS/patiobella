...
class RestaurantTableController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Fetch active unpaid orders keyed by table_id so the blade can
        // detect which tables have a printed bill awaiting payment.
        $orders = SalesOrder::whereIn('table_id', $tables->pluck('id'))
            ->where('payment_status', 'unpaid')
            ->where('status', '!=', 'cancelled')
            ->get()
            ->keyBy('table_id');

        return view('management.tables.index', compact('tables', 'locations', 'capacities', 'orders'));
    }
