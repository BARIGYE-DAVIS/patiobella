...
<style>
    ...
    .grn-card:hover {
        ...
        box-shadow: 0 8px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.02);
    }
    .status-badge {
        display: inline-flex;
        align-items: center;
...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 grn-card">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 grn-card">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600">{{ $grns->where('status', 'completed')->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 grn-card">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-amber-600">{{ $grns->where('status', 'pending')->count() }}</p>
                </div>
        ...
        <div class=" header  px-6 py-4 flex justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-emerald-100 mt-0.5">Manage and track all received goods</p>
                </div>
            </div>
            <a href="{{ route('procurement.goods-received.create') }}"
               class="inline-flex items-center gap-2 px-4 py-2 bg-white text-emerald-700 rounded-lg hover:bg-emerald-50 transition-all duration-200 shadow-sm hover:shadow-md text-sm font-semibold">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                <div class="flex-1 relative">
                    ...
                    <input type="text" id="searchInput"
                           ...
                           class="search-input w-full pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all duration-200">
                </div>
                <div class="flex gap-3">
                    <div class="relative">
                        <i class="fas fa-filter absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 text-sm"></i>
                        <select id="statusFilter" class="filter-select pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all duration-200">
                    ...
                    <button id="clearFilters" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg text-sm transition-all duration-200">
                ...
                <tbody id="tableBody">
                    ...
                    <tr class="grn-row"
                        ...
                        <td class="text-center">
                            @php
                                $statusClass = $grn->status == 'completed' ? 'status-completed' : ($grn->status == 'pending' ? 'status-pending' : 'status-cancelled');
                            @endphp
                    ...
                    <tr id="noDataRow">
                        <td colspan="6" class="empty-state">
                            ...
                            <p class="text-gray-500 mb-2">No Goods Received Notes found</p>
                            <a href="{{ route('procurement.goods-received.create') }}" class="text-emerald-600 hover:underline text-sm">
                ...
                if (!noDataRow) {
                    ...
                    newNoDataRow.innerHTML = `
                        <td colspan="6" class="empty-state">
                            ...
                            <button id="clearFiltersEmpty" class="mt-3 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm hover:bg-emerald-700 transition">Clear Filters</button>
                        </td>
                    `;
