{{-- resources/views/management/stock-counts/create.blade.php --}}
...
<div class="space-y-6">
    ...
    <div class="border-b border-gray-200 bg-white rounded-t-xl shadow-sm px-6 pt-4">
        <nav class="-mb-px flex space-x-8">
            <a href="{{ route('management.stock-counts.create', ['type' => 'store']) }}"
            ...
            <a href="{{ route('management.stock-counts.create', ['type' => 'department']) }}"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <form method="POST" action="{{ route('management.stock-counts.store') }}" id="stockCountForm">
            @csrf
...
<script>
    ...
    function fetchItemStock(index, itemId) {
        ...
        fetch(`{{ url('management/stock-counts/get-item-stock') }}/${itemId}`)
            .then(r => r.json())
            .then(data => {
                if (data.success) {
    ...
    function loadDepartmentItems(departmentId) {
        ...
        fetch(`{{ url('management/stock-counts/get-department-items') }}/${departmentId}`)
            .then(r => r.json())
            .then(data => {
                tbody.innerHTML = '';
    ...
    document.getElementById('stockCountForm').addEventListener('submit', function(e) {
        const hasCount = [...document.querySelectorAll('[name*="physical_quantity"]')]
            .some(el => parseFloat(el.value) > 0);
        if (!hasCount) {
            e.preventDefault();
            alert('Please add at least one item with a valid physical count.');
        }
    });
</script>
@endpush
