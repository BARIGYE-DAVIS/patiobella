<?php
// app/Http/Controllers/Restaurant/CashierPosController.php
...
class CashierPosController extends Controller
{
 ...
 /**
 * Point of Sale (POS) Screen
 * Shows menu items + inventory items that have stock from requisitions
 */
public function pos()
{
    try {
        ...
        // Get ONLY inventory items that have stock from requisitions for this department
        ...
        // Calculate current stock and collect unique items with stock > 0
        $sellableItems = [];
        $currentStock = [];
...
public function markAsPaid($id, Request $request)
{
    ...
    /**
 * Process direct sale (without invoice - legacy)
 */
...
/**
 * Create invoice (pending payment) - DEDUCT STOCK FOR INVENTORY ITEMS
 */
public function createInvoice(Request $request)
{
    ...
    /**
     * Store Order (Direct sale with payment)
     */
        ...
        try {
            ...
            foreach ($request->items as $item) {
                SalesOrderItem::create([
                    ...
                    'item_name' => MenuItem::find($item['menu_item_id'])->name,
