<?php
// app/Http/Controllers/Kitchen/KitchenStockController.php
...
class KitchenStockController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            // Get all requisition items for KITCHEN department
            ...
            Log::info('Kitchen stock accessed', [
                'user_id' => $user->id,
                'total_items' => $totalItems,
                'total_stock_value' => $totalStockValue
            ]);

            return view('kitchen.stock.index', compact(
                'stockData',
                'totalItems',
                'totalIssued',
                'totalConsumed',
            ...
            Log::error('Kitchen stock error', [
                'user_id' => Auth::id(),
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            return redirect()->route('kitchen.dashboard')->with('error', 'Failed to load stock: ' . $e->getMessage());
        }
    }
    ...
    public function show($id)
    {
        try {
            ...
            return view('kitchen.stock.show', compact(
                'inventoryItem',
                'totalIssued',
                'totalConsumed',
                'totalSold',
            ...
            Log::error('Kitchen stock show error', [
                'user_id' => Auth::id(),
                'item_id' => $id,
                'error' => $e->getMessage()
            ]);
            return redirect()->route('kitchen.stock.index')->with('error', 'Item not found.');
        }
    }
...
public function exportPdf(Request $request)
{
    try {
        ...
        foreach ($requisitionItems as $item) {
            ...
            if (!isset($stockData[$itemId])) {
                $stockData[$itemId] = [
                    ...
                    'unit' => $inventoryItem->base_unit ?? 'units',
                    'category_id' => $inventoryItem->category_id,
                    'category_name' => $inventoryItem->category->name ?? 'N/A',
                    'issued' => 0,
                    'consumed' => 0,
                    'returned' => 0,
                    'current_stock' => 0,
        ...
        if ($request->filled('stock_status')) {
            ...
            $stockData = array_filter($stockData, function($item) use ($status) {
                if ($status === 'in_stock') return $item['current_stock'] > 0;
                if ($status === 'low_stock') return $item['current_stock'] > 0 && $item['current_stock'] <= 10;
                if ($status === 'out_of_stock') return $item['current_stock'] <= 0;
                return true;
            });
        }
        ...
        $pdf = Pdf::loadView('kitchen.stock.pdf', $data);
        $pdf->setPaper('a4', 'landscape');

        return $pdf->download('kitchen_stock_report_' . date('Y-m-d_His') . '.pdf');
