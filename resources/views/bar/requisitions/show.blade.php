...
<div id="requisitionPrintArea">
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden print-hide">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center flex-wrap gap-3">
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('bar.requisitions.index') }}" class="text-gray-600 hover:text-gray-800 text-sm font-medium transition flex items-center gap-1">
    ...
    <div class="print-header hidden">
        <div style="text-align: center; margin-bottom: 20px; border-bottom: 2px solid #f97316; padding-bottom: 15px;">
            <h1 style="margin: 0; color: #1f2937; font-size: 24px;">BAR REQUISITION FORM</h1>
            <p style="margin: 5px 0 0; color: #6b7280; font-size: 12px;">Official Bar Requisition Document</p>
        </div>
    </div>
    ...
    <div class="p-6">
        ...
        @php
            ...
            $totalRemaining = $totalIssued - ($totalSold + $totalReturned);
        @endphp

        <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                <div>
                    <i class="fas fa-box-open text-blue-400 text-lg mb-1"></i>
                    <p class="text-xs text-gray-500">Total Issued</p>
                    <p class="text-xl font-bold text-blue-600">{{ number_format($totalIssued, 2) }}</p>
                </div>
                <div>
                    <i class="fas fa-shopping-cart text-red-400 text-lg mb-1"></i>
                ...
                <div>
                    <i class="fas fa-chart-line text-green-400 text-lg mb-1"></i>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
                <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">
                    <i class="fas fa-truck mr-1"></i> Issuance Information
                </h4>
                <div class="space-y-2">
                    <div class="flex flex-wrap">
                        <span class="text-sm font-medium text-gray-500 w-32">Taken By:</span>
                        <span class="text-sm font-semibold text-blue-600"><i class="fas fa-user mr-1"></i>{{ $stockMovement->taken_by ?? 'Not recorded' }}</span>
                    </div>
                    <div class="flex flex-wrap">
                        <span class="text-sm font-medium text-gray-500 w-32">Issued Date:</span>
                        <span class="text-sm text-gray-800"><i class="far fa-calendar-alt mr-1"></i>{{ $stockMovement->created_at ? $stockMovement->created_at->format('F d, Y g:i A') : '—' }}</span>
                    </div>
                    <div class="flex flex-wrap">
                        <span class="text-sm font-medium text-gray-500 w-32">Issued By (Store):</span>
                        <span class="text-sm text-gray-800"><i class="fas fa-store mr-1"></i>{{ $stockMovement->createdBy->name ?? 'System' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div>
                <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">
                    <i class="fas fa-undo-alt mr-1"></i> Return Information
                </h4>
                <div class="space-y-2">
                    <div class="flex flex-wrap">
                        <span class="text-sm font-medium text-gray-500 w-32">Returned By:</span>
                        <span class="text-sm font-semibold text-purple-600"><i class="fas fa-user mr-1"></i>{{ $returnMovement->returned_by ?? 'Not recorded' }}</span>
                    </div>
                    <div class="flex flex-wrap">
                        <span class="text-sm font-medium text-gray-500 w-32">Return Date:</span>
                        <span class="text-sm text-gray-800"><i class="far fa-calendar-alt mr-1"></i>{{ $returnMovement->created_at ? $returnMovement->created_at->format('F d, Y g:i A') : '—' }}</span>
                    </div>
                </div>
            </div>
            @endif
        ...
        <div class="mb-6">
            <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">
                <i class="fas fa-sticky-note mr-1"></i> Bar Notes
            </h4>
            ...
            <div class="overflow-x-auto border border-gray-200 rounded-lg">
                <table class="min-w-full text-sm">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            <th class="text-left px-4 py-3 font-semibold text-gray-600 w-[20%]">Item</th>
                            <th class="text-left px-4 py-3 font-semibold text-gray-600 w-[8%]">Unit</th>
                            <th class="text-center px-4 py-3 font-semibold text-gray-600 w-[12%]">Requested</th>
                            <th class="text-center px-4 py-3 font-semibold text-green-600 bg-green-50 w-[12%]">Issued</th>
                            <th class="text-center px-4 py-3 font-semibold text-red-600 bg-red-50 w-[12%]">Sold</th>
                            <th class="text-center px-4 py-3 font-semibold text-purple-600 bg-purple-50 w-[12%]">Returned</th>
                            <th class="text-center px-4 py-3 font-semibold text-blue-600 bg-blue-50 w-[12%]">Remaining</th>
                            <th class="text-left px-4 py-3 font-semibold text-gray-600 w-[14%]">Notes</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        ...
                        @php
                            ...
                            $remaining = $issued - ($sold + $returned);
                        @endphp
                            ...
                            <td class="px-4 py-3 text-center">
                                @if($sold > 0)
                                    <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-700">
                                        <i class="fas fa-shopping-cart mr-1 text-xs"></i>{{ number_format($sold, 2) }}
                                    </span>
                                @else
